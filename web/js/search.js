...
(function (window, document, undefined) {
	...
	let createResultsHTML = function (results) {
		let html = '<p>Found ' + results.length + ' matching page(s)</p>';
	...
	let search = function (query) {
		...
		searchModels.forEach(function (article) {
			if (reg.test(article.title)) return priority1.push(article);
			if (reg.test(article.content)) priority1.push(article);
		});
		
		searchLists.forEach(function (article) {
			if (reg.test(article.title)) return priority2.push(article);
	...
	if (!form || !input || !resultList || !searchLists || !searchModels || !searchQuotes) return;
